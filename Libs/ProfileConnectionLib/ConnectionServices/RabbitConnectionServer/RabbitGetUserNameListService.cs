    ...
    where TModel : BaseEntityDal<Guid>
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.Received += async (model, ea) =>
        {
            ...
            var getUserDeserializedData = JsonConvert.DeserializeObject<UserNameProfileApiRequest>(message)
                                             ?? throw new Exception($"Ошибка при десериализации {typeof(UserNameProfileApiRequest)}");
